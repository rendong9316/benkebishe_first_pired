...
clear; close all; clc;
addpath('config', 'utils', 'simulation', 'registration', 'ukf', 'fusion', 'visualization', 'io', 'tracker');
...
% 多目标真值
truths = cell(params.num_aircraft, 1);
for a = 1:params.num_aircraft
    tt = true_tracks{a};
    truths{a} = struct('time_sec', tt(:,5), 'lat', tt(:,2), ...
        'lon', tt(:,1), 'lon_rate', tt(:,3), 'lat_rate', tt(:,4));
end

outf = fullfile('results', sprintf('simulation_multi_%s.mat', datestr(now, 'yyyymmdd_HHMMSS')));
save(outf, 'detList_R1', 'detList_R2', 'trackList_R1', 'trackList_R2', ...
    'trackSnapshots_R1', 'trackSnapshots_R2', ...
    'calib', 'scene', 'truths', 'params', ...
    'total_r1', 'total_r2', 'ac_det_counts_r1', 'ac_det_counts_r2');
fprintf('数据已保存: %s\n', outf);
fprintf('\nDone.\n');
